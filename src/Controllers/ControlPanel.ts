...
export class ControlPanel {
  ...
  defineHotkeys() {
    $( "body" ).keydown( e => {
      ...
      case "v": // toggle hud
        if ( this.no_hud ) {
          const control_panel_display = this.control_panel_active ? "grid" : "none",
                hot_control_display = !this.control_panel_active ? "block" : "none";
  ...
  defineWorldControls() {
    ...
    $( "#resize" ).click( () => {
      var cell_size = $( "#cell-size" ).val(),
          fill_window = $( "#fill-window" ).is( ":checked" );
    ...
    $( "#reset-with-editor-org" ).click( () => {
      ...
      const center = env.grid_map.getCenter(),
            org = this.editor_controller.env.getCopyOfOrg();
  ...
  defineHyperparameterControls() {
    ...
    $( "#food-drop-rate" ).change( () => {
      Hyperparams.foodDropProb = $( "#food-drop-rate" ).val();
    } );
    $( "#extra-mover-cost" ).change( () => {
    ...
    $( "#save-controls" ).click( () => {
      const data = "data:text/json;charset=utf-8," + encodeURIComponent( JSON.stringify( Hyperparams ) ),
            downloadEl = document.getElementById( "download-el" );
  ...
  defineModeControls() {
    ...
    $( ".tabnav-item" ).click( () => {
      ...
      case "food-drop":
        ...
        break;
      case "wall-drop":
        this.setMode( Modes.WallDrop );
        break;
      case "click-kill":
        this.setMode( Modes.ClickKill );
        break;
      case "select":
