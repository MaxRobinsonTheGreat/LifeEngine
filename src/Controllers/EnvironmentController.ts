...
export class EnvironmentController extends CanvasController{
  ...
  defineZoomControls() {
    var scale = 1,
        zoom_speed = 0.5;
    const el = <HTMLElement>document.querySelector( "#env-canvas" );
    ...
    el.onwheel = function zoom( event ) {
      ...
      var cur_top = parseInt( $( "#env-canvas" ).css( "top" ) ),
          cur_left = parseInt( $( "#env-canvas" ).css( "left" ) ),

          diff_x = ( this.canvas.width / 2  - this.mouse_x ) * ( scale - this.scale ),
          diff_y = ( this.canvas.height / 2 - this.mouse_y ) * ( scale - this.scale );
  ...
  randomizeWalls( thickness = 1 ) {
    ...
    for ( let r = 0; r < this.env.num_rows; r++ ) 
      for ( let c = 0; c < this.env.num_cols; c++ ) {
        const xval = c / this.env.num_cols * ( resolution / this.env.renderer.cell_size * ( this.env.num_cols / this.env.num_rows ) ),
        ...
        if ( n > noise_threshold && n < noise_threshold + thickness / resolution ) {
  ...
  performModeAction() {
    if ( WorldConfig.headless && this.mode != Modes.Drag )
      return;
    var mode = this.mode,
        right_click = this.right_click,
        left_click = this.left_click;
