...
export class Organism {
  ...
  reproduce() {
    ...
    var direction = Directions.getRandomScalar(),
        direction_c = direction[ 0 ],
        direction_r = direction[ 1 ],
        offset = ( Math.floor( Math.random() * 3 ) ),
        basemovement = this.anatomy.birth_distance,
        new_c = this.c + ( direction_c * basemovement ) + ( direction_c * offset ),
        new_r = this.r + ( direction_r * basemovement ) + ( direction_r * offset );

    // console.log(org.isClear(new_c, new_r, org.rotation, true))
    if ( org.isClear( new_c, new_r, org.rotation ) && org.isStraightPath( new_c, new_r, this.c, this.r, this ) ){
      org.c = new_c;
      org.r = new_r;
  ...
  attemptMove() {
    var direction = Directions.scalars[ this.direction ],
        direction_c = direction[ 0 ],
        direction_r = direction[ 1 ],
        new_c = this.c + direction_c,
        new_r = this.r + direction_r;

    if ( this.isClear( new_c, new_r ) ) {
      for ( var cell of this.anatomy.cells ) {
        var real_c = this.c + cell.rotatedCol( this.rotation ),
            real_r = this.r + cell.rotatedRow( this.rotation );
  ...
  getRealCell( local_cell, c = this.c, r = this.r, rotation = this.rotation ){
    var real_c = c + local_cell.rotatedCol( rotation ),
        real_r = r + local_cell.rotatedRow( rotation );
